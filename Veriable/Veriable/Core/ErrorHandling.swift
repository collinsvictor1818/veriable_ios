...
enum AppError: UserFacingError {
    ...
    var message: String {
        ...
        case .unknown(let error): return "Something went wrong. Please try again later. Details: \(error.localizedDescription)"
        }
    }
}
...
enum NetworkError: UserFacingError {
    ...
    var message: String {
        ...
        case .serverError(let statusCode): return "The server returned an error with status code \(statusCode)."
...
enum DataError: UserFacingError {
    ...
    var message: String {
        switch self {
        case .decodingFailed(let error): return "Failed to decode the data. \(error.localizedDescription)"
        case .encodingFailed(let error): return "Failed to encode the data. \(error.localizedDescription)"
        }
    }
}
