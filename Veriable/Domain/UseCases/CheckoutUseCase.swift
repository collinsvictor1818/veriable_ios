...
final class CheckoutUseCase: CheckoutUseCaseProtocol {
    ...
    func execute(items: [CartItem]) async throws {
        ...
        let orderResponse: OrderResponse = try await apiClient.request(OrderAPI.createOrder(userId: userId, total: total, items: items))
        ...
        for item in items {
            ...
            guard let httpResponse = response as? HTTPURLResponse,
                ...
                throw AppError.network(.serverError(statusCode: (response as? HTTPURLResponse)?.statusCode ?? 0))
            }
        }
