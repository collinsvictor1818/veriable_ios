            ...
            List(products) { product in
                HStack(spacing: 12) {
                    AsyncImage(url: product.imageUrl) { phase in
                        ...
                        case .failure:
                            Image(systemName: "photo").resizable().scaledToFit().frame(width: 56, height: 56).foregroundColor(.secondary)
                        @unknown default:
                            EmptyView()
                        }
                    }
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text(product.price, format: .currency(code: "USD")).font(.subheadline).foregroundColor(.secondary)
                    }
                    Spacer()
                    Button(action: { onAdd(product) }) { Image(systemName: "plus").font(.headline).padding(8) }
