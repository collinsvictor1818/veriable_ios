  ...
  AVCaptureVideoDataOutputSampleBufferDelegate
{
  ...
  private func setupCaptureSession() {
    ...
    if captureSession.canAddInput(videoInput) {
      ...
      delegate?.didFailWithError(
        NSError(
          domain: "", code: -2, userInfo: [NSLocalizedDescriptionKey: "Could not add video input"]))
      return
    }
    ...
    if captureSession.canAddOutput(metadataOutput) {
      ...
      if captureSession.canAddOutput(videoOutput) {
        ...
        delegate?.didFailWithError(
          NSError(
            domain: "", code: -4,
            userInfo: [NSLocalizedDescriptionKey: "Could not add video data output"]))
      }
    } else {
      delegate?.didFailWithError(
        NSError(
          domain: "", code: -3,
          userInfo: [NSLocalizedDescriptionKey: "Could not add metadata output"]))
      return
    }
  ...
  func setCameraPosition(_ position: AVCaptureDevice.Position) {
    ...
    if let connection = (previewLayer?.connection ?? captureSession.connections.first),
  ...
  func setTorch(on: Bool) {
    guard let deviceInput = captureSession?.inputs.compactMap({ $0 as? AVCaptureDeviceInput }).first
    else { return }
